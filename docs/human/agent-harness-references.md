...
- Use isolated subagent or forked context for research-heavy, log-heavy, or
  ...
  route in `skills/skill-runner/references/codex-delegation.md`; native v2 is the
  lightweight read-only path after a no-edit probe, while tmux/`skill-runner`
  remains the durable and mutation-safe path.
...
| [Codex subagents](https://developers.openai.com/codex/subagents) | The installed Codex CLI's native v2 passed this harness's spawn, result-delivery, lifecycle, nesting, and bounded-concurrency probes. Treat the host capability probe and `skills/skill-runner/references/codex-delegation.md` as the route authority; native v2 is for lightweight read-only work, while tmux/`skill-runner` remains the durable and mutation-safe adapter. Re-probe after CLI upgrades. |
...
| [Ege Celebi: STE writing experiment](https://github.com/woosal1337/blog/tree/main/videos/ep01-the-cure-for-ai-slop) | Controlled writing rules reduce mechanical prose violations, but the experiment does not measure plan correctness or semantic retention. Use the adapted rules as a post-reconciliation form check with protected-contract guards, not as a planning owner or quality gate. |
...
| [Matt Pocock `research`](https://github.com/mattpocock/skills/blob/main/skills/engineering/research/SKILL.md) | Research claims should resolve to high-trust primary sources and retain citations. The upstream skill is intentionally thin, so this repo keeps the invariant without adding another external install. |
| [Exa research orchestrator](https://github.com/exa-labs/exa-mcp-server/blob/main/skills/search/SKILL.md) | Complex research benefits from explicit decomposition, deduplication, source-quality checks, counterevidence, and a targeted gap pass. Keep those methods host-neutral instead of hardcoding Exa or Claude subagents. |
| [ModelScope ms-agent deep research](https://github.com/modelscope/ms-agent/blob/main/ms-agent-skills/references/deep-research.md) | File-backed evidence and report stages make long research auditable, but a full research runtime brings model, search-provider, Python, and remote-code trust costs. Reuse the evidence contract locally and leave heavyweight engines optional. |
