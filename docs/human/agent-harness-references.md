...
| [Claude Code issue #11450: Prompt Review Agent](https://github.com/anthropics/claude-code/issues/11450) | Preflight prompt review should happen before executor context is polluted. `$intuitive-contract` carries this lesson by making missing context, clarifying questions, and the approval-ready context package explicit before `$intuitive-flow` or `skill-runner` starts implementation. |
